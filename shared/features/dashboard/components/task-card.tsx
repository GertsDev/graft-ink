...
export default function TaskCard({ task }: Props) {
  ...
  return (
    ...
    >
      <Card
        className={`group relative overflow-hidden shadow-sm duration-200 hover:shadow-md motion-safe:transition-all`}
      >
        {/* Left accent that thickens slightly on hover (no scale). Uses neutral when no color */}
        <div
          className={`absolute top-0 left-0 h-full w-[3px] motion-safe:transition-all ${
            isColorPickerOpen ? "w-1.5" : ""
          } group-hover:w-1.5 ${task.color ? "" : "bg-border"}`}
          style={
            task.color
              ? { backgroundColor: TASK_COLORS[task.color].hex }
              : undefined
          }
          aria-hidden="true"
        />

        {/* Removed full-card hover tint per UX decision; only left accent highlights */}
        {/* Celebration overlay */}
        <AnimatePresence>
          {showCelebration && (
            <motion.div
              ...
              className="from-analyze-1/20 to-analyze-2/20 pointer-events-none absolute inset-0 z-10 flex items-center justify-center bg-gradient-to-r backdrop-blur-sm"
            >
              <motion.div
                initial={{ scale: 0, rotate: -180 }}
                animate={{ scale: 1, rotate: 0 }}
                className="text-analyze-1 flex items-center gap-2 rounded-full bg-white/90 px-4 py-2 text-lg font-bold shadow-lg"
              >
        ...
        {(task.todayTime ?? 0) > 0 && (
          <div className="from-analyze-1/5 to-analyze-2/5 absolute inset-0 z-0 bg-gradient-to-br" />
        )}
            ...
            <div className="w-full">
              {isEditing ? (
                ...
                <>
                  ...
                  <p className="min-h-[1.25rem] text-sm text-gray-500 transition-colors duration-200">
                    {task.topic || ""}
                  </p>
                </>
              )}
          ...
          <div className="flex items-center justify-between">
                    ...
                    <motion.div key={d}>
                      <Button
                        ...
                        className={`relative overflow-hidden transition-all duration-200 hover:scale-110 active:scale-95 ${
                          justAddedTime === d
                            ? "bg-analyze-1 animate-bounce text-white shadow-lg"
                            : ""
                        }`}
                      >
                        ...
                        {justAddedTime === d && (
                          <motion.div
                            ...
                            className="bg-analyze-1/30 absolute inset-0 rounded-md"
                          />
                        )}
                        <motion.span
            ...
            <div className="text-right text-sm">
              <motion.p
                ...
                animate={
                  justAddedTime
                    ? { scale: [1, 1.1, 1], color: "var(--analyze-1)" }
                    : {}
                }
              ...
              <motion.p
                ...
                className="text-analyze-2 mt-1 text-xs font-medium"
              >
